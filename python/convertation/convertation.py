...
class BrainCleaner3D(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.sigma_slider.setValue(12)  # 0-30 -> 0-3.0
    ...
    def generate_3d(self):
        ...
        try:
            ...
            # 8. Децимация (упрощение) меша для уменьшения размера, если нужно
            # Раскомментируйте для уменьшения количества треугольников
            ...
            # 9. Визуализация с улучшенным освещением
            self.plotter.clear()

            # Вычисляем кривизну для лучшей визуализации извилин
            try:
                curvature = self.last_mesh.curvature()
                ...
                # Если не удалось вычислить кривизну, используем простой цвет
                self.plotter.add_mesh(self.last_mesh,
                                      color="#E6BE8A",
                                      lighting=True,
                                      smooth_shading=True,
            ...
            self.plotter.enable_eye_dome_lighting()
            self.plotter.add_light(pv.Light(position=(5, 5, 10), light_type='scene light'))
    ...
    def export_stl(self):
        ...
        path, selected_filter = QFileDialog.getSaveFileName(
            ...
            "Save 3D Model",
            "brain_model.stl",
            "STL Files (*.stl);;PLY Files (*.ply);;OBJ Files (*.obj)"
        )
