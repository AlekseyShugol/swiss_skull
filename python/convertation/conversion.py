...
class BrainCleanerLogic:
    ...
    def fix_orientation_and_center(self, mesh, center_model=True):
        ...
        DICOM: ось Z идёт снизу вверх (ноги->голова)
        После marching cubes: X,Y,Z соответствуют осям массива
        Нужно получить: пациент стоит, смотрит вперёд
        """
        # Сохраняем оригинальный размер для отладки
        original_bounds = mesh.bounds
        print(f"Original bounds: {original_bounds}")

        # Медицинские данные в массиве numpy:
        # ось 0 (Z) = superior-inferior (голова-ноги)
        # ось 1 (Y) = anterior-posterior (перед-зад)
        # ось 2 (X) = left-right (лево-право)

        # После marching cubes с spacing=(spacing_z, spacing_y, spacing_x):
        # X в меше = ось Z массива (superior-inferior)
        # Y в меше = ось Y массива (anterior-posterior)
        # Z в меше = ось X массива (left-right)

        # Поэтому меняем оси местами для правильной ориентации
        # X (left-right) <- Z (ось X массива)
        # Y (superior-inferior) <- X (ось Z массива)
        # Z (anterior-posterior) <- Y (ось Y массива)

        points = mesh.points.copy()

        # Перестановка координат для правильной ориентации
        ...
        # Теперь пациент должен быть ориентирован правильно:
        # X = left-right
        # Y = up-down (superior-inferior)
        # Z = front-back (anterior-posterior)
